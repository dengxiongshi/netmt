...
int cmd_main(int argc, char **argv, CMD_INIT_OTHER pfun)
{
	...
	if (!GetAbsPath("../etc/netmtserver.ini", szIniAbsPath, sizeof(szIniAbsPath)))
	{
		printf("\nGet cfg-file failed! file='%s'\n", "../etc/netmtserver.ini");
		return 1;
	}
	if (!CGetValueInt(szIniAbsPath, "vty", "vtyport", 2188, &vty_port, ""))
	{
		printf("vty:vtyport error!");
	}
	...
	vty_serv_sock(vty_addr, vty_port ? vty_port : 2188, "./");
